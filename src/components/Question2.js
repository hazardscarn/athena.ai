...
const Question2 = ({ onNext, previousAnswers }) => {
  const [answer, setAnswer] = useState('');
  const [wordCount, setWordCount] = useState(0);
  const [isValidWordCount, setIsValidWordCount] = useState(false);

  useEffect(() => {
    if (previousAnswers && previousAnswers.q2) {
      setAnswer(String(previousAnswers.q2));
    }
  }, [previousAnswers]);
  ...
  return (
    ...
    >
      <h2 className="text-3xl font-bold mb-6 text-indigo-700">Future Goals</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="futureGoals">
            Where do you see yourself in 1 year? (10-200 words)
          </label>
          <textarea
            id="futureGoals"
            value={answer}
